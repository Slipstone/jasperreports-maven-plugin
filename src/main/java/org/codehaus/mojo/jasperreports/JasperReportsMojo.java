...
public class JasperReportsMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		final SourceMapping mapping = new SuffixMapping(sourceFileExt,
				outputFileExt);

		final Set staleSources = scanSrcDir(mapping);
	...
	protected void compile(final Set files, final SourceMapping mapping)
			throws MojoFailureException, MojoExecutionException {
		final String classpath = buildClasspathString(classpathElements,
				additionalClasspath);
		...
		try {
			JRProperties
					.setProperty(JRProperties.COMPILER_CLASSPATH, classpath);
			JRProperties.setProperty(JRProperties.COMPILER_TEMP_DIR,
					javaDirectory.getAbsolutePath());
			JRProperties.setProperty(JRProperties.COMPILER_KEEP_JAVA_FILE,
					keepJava);
			JRProperties.setProperty(JRProperties.COMPILER_CLASS, compiler);
			JRProperties.setProperty(JRProperties.COMPILER_XML_VALIDATION,
					xmlValidation);

			for (final Iterator i = additionalProperties.keySet().iterator(); i
					.hasNext();) {
				final String key = (String) i.next();
				final String value = (String) additionalProperties.get(key);
			...
			while (it.hasNext()) {
				...
				final String srcName = getPathRelativeToRoot(src);
				try {
					// get the single destination file
					final File dest = (File) mapping
							.getTargetFiles(outputDirectory, srcName)
							.iterator().next();

					final File destFileParent = dest.getParentFile();
					if (!destFileParent.exists()) {
						if (destFileParent.mkdirs()) {
							getLog().debug(
									"Created directory " + destFileParent);
						} else {
	...
	private String getPathRelativeToRoot(final File file)
			throws MojoExecutionException {
		try {
			final String root = this.sourceDirectory.getCanonicalPath();
	...
	protected String buildClasspathString(final List classpathElements,
			final String additionalClasspath) {
		final StringBuffer classpath = new StringBuffer();
		final Iterator it = classpathElements.iterator();
		while (it.hasNext()) {
			final String cpElement = (String) it.next();
	...
	private void checkDir(final File dir, final String desc,
			final boolean isTarget) throws MojoExecutionException {
	...
	private ClassLoader getClassLoader(final ClassLoader classLoader)
			throws MojoExecutionException {
		final List classpathURLs = new ArrayList();

		for (int i = 0; i < classpathElements.size(); i++) {
			final String element = (String) classpathElements.get(i);
			try {
		...
		if (additionalClasspath != null && additionalClasspath.length() > 0) {
			final String[] elements = additionalClasspath
		...
		final URL[] urls = (URL[]) classpathURLs.toArray(new URL[classpathURLs
				.size()]);
